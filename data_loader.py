...
#         self.viz = viz
#         self.debug = debug
#         self.gaussianTransformer17 = GaussianTransformer()
#         self.gaussianTransformer20 = GaussianTransformer()
#
#
...
#         if pursedo_len == 0:
#             return 0.
#         return (real_len - min(real_len, abs(real_len - pursedo_len))) / (real_len + 1e-8)
#
...
#         pursedo_bboxes = watershed2(bgr_region_scores,water_image.copy())
#
...
#
#             width_per_char = width / (len(word)+1e-8)
...
#             region_scores = self.gaussianTransformer17.generate_region(image.shape, character_bboxes)
#             affinity_scores, affinity_bboxes = self.gaussianTransformer20.generate_affinity(image.shape,
#                                                                                           character_bboxes,
#                                                                                           words)
#         if self.viz:
...
#         self.net = net
#         self.img_folder = os.path.join(icdar2015_folder, 'images/ch8_training_images')
...
#     net = CRAFT()
#     net.load_state_dict(
#         copyStateDict(torch.load('/home/jiachx/gitcode/CRAFT-reimplement/checkpoints/synth80k_lower_loss_vgg.pth')))
#     net = net.cuda()
#     net = torch.nn.DataParallel(net)
#     net.eval()
#     dataloader = ICDAR2015(net, '/home/jiachx/publicdatasets/icdar2015/icdar2015train', target_size=640, viz=True)
#     train_loader = torch.utils.data.DataLoader(
#         dataloader,
#         batch_size=1,
#         shuffle=False,
...
#         drop_last=True,
#         pin_memory=True)
#     for index, (opimage, region_scores, affinity_scores, confidence_mask) in enumerate(train_loader):
#         pass
...
#
#         if len(imgs[idx].shape) == 3:
#             imgs[idx] = imgs[idx][i:i + th, j:j + th, :]
#         else:
#             imgs[idx] = imgs[idx][i:i + tw, j:j + tw]
#
#     return imgs
...
class craft_base_dataset(data.Dataset):
    def __init__(self, target_size=768, viz=False, debug=False):
        ...
        self.viz = viz
        self.debug = debug
        self.gaussianTransformer = GaussianTransformer(imgSize=1024, region_threshold=0.3, affinity_threshold=0.2)
...
if __name__ == '__main__':
    ...
    net.load_state_dict(
        copyStateDict(torch.load('/ic15_iter_1300.pth')))
    net = net.cuda()
    net = torch.nn.DataParallel(net)
    net.eval()
    dataloader = ICDAR2015(net, '/icdar2015/icdar2015train', target_size=640, viz=True)
    train_loader = torch.utils.data.DataLoader(
        dataloader,
        batch_size=1,
        shuffle=False,
